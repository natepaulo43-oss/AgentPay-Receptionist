...
exports.enforceChatRateLimit = enforceChatRateLimit;
exports.parseConversationHistory = parseConversationHistory;
exports.chatResponseFor = chatResponseFor;
...
function enforceChatRateLimit(event) {
    const key = getClientKey(event);
    const current = Date.now();
    const hit = chatHits.get(key);
    if (!hit || current - hit.windowStartedAt > config_1.CHAT_RATE_WINDOW_MS) {
        chatHits.set(key, { windowStartedAt: current, count: 1 });
        return true;
    }
    hit.count += 1;
    return hit.count <= config_1.CHAT_RATE_LIMIT;
}
...
function chatResponseFor(input) {
    ...
    if (asksHours(lower)) {
        return freeResponse('Miami Elite Auto Detail is open Monday through Saturday, 8 AM to 6 PM. Same-day priority holds are available when capacity allows.', 'business_hours', leadFields, 'Answer free business-hours questions. Do not request payment.');
    }
    if (asksLocation(lower)) {
        return freeResponse('We serve Miami, Brickell, Wynwood, Coral Gables, and Miami Beach. Mobile appointments can be held once the vehicle, service, and slot are clear.', 'service_area', leadFields, 'Answer free location questions. Ask for a paid action only if the customer asks to reserve capacity.');
    }
    if (asksPrice(lower) && !requestedPaidAction) {
        const service = findService(combinedText);
        return freeResponse(service
            ...
            : 'Pricing depends on service, vehicle size, and condition. I can answer questions free; payment is only needed to hold capacity or submit a verified request.', 'pricing_question', leadFields, 'Keep pricing informational and free until the customer requests a business action.');
    }
    if (isGreeting(lower)) {
        return freeResponse('Hi, I am the AI front desk for Miami Elite Auto Detail. What service are you looking for today?', 'greeting', leadFields, 'Start qualification with one question. Do not claim to be human.');
    }
...
function freeResponse(reply, intent, leadFields, agentInstructions) {
    return {
        reply,
        intent,
