...
exports.parseRouteConfig = parseRouteConfig;
// ---------------------------------------------------------------------------
 ...
 */
function parseRouteConfig(json) {
    let parsed;
    try {
...
// Internal Validators
// ---------------------------------------------------------------------------
function validateRouteConfig(input) {
...
function validateRouteEntry(input, index) {
...
function validateAccessPolicy(input, prefix) {
    if (input === null || typeof input !== 'object' || Array.isArray(input)) {
        return fail(`${prefix}: must be a non-null object`);
    }
    const obj = input;
    ...
    if (condResult.success === false) {
        return { success: false, error: condResult.error };
    }
    ...
    if (action !== 'block' && !PRICE_PATTERN.test(action)) {
        return fail(`${prefix}.action: must be "block" or a non-negative price string (e.g., "0", "0.001", "10"), got "${action}"`);
    }
    return {
        success: true,
        policy: {
 ...
 */
function validateConditionExpression(input, prefix, depth = 0) {
    ...
    if (typeof input === 'object' && input !== null) {
        ...
        if (keys.length !== 1 || !['and', 'or', 'not', 'namespace'].includes(keys[0])) {
            return fail(`${prefix}: condition object must have exactly one key: "and", "or", "not", or "namespace"`);
        }
        const key = keys[0];
        if (key === 'namespace') {
        ...
        if (key === 'not') {
            ...
            return {
                success: true,
                condition: { not: subResult.condition },
            };
        }
        ...
        for (let i = 0; i < arr.length; i++) {
            const subResult = validateConditionExpression(arr[i], `${prefix}.${key}[${i}]`, depth + 1);
            if (!subResult.success) {
                return subResult;
            }
...
//# sourceMappingURL=route-config-validator.js.map