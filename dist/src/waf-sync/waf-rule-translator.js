...
exports.translateRouteConfig = translateRouteConfig;
const route_pattern_translator_1 = require("./route-pattern-translator");
 ...
 *
 */
function translateRouteConfig(config) {
    const rules = [];
    ...
    for (let routeIndex = 0; routeIndex < config.routes.length; routeIndex++) {
        ...
        const uriStatement = (0, route_pattern_translator_1.toWafStatement)(route.pattern);
        for (let policyIndex = 0; policyIndex < route.policies.length; policyIndex++) {
            ...
            if (action !== 'block') {
                rule.ruleLabels = [constants_1.RouteMatchedLabel.KEY];
            }
            rules.push(rule);
 ...
 */
function wrapWithScopeDown(originalStatement) {
    const notAlreadyMatched = {
        notStatement: {
            statement: {
                labelMatchStatements: [{ scope: constants_1.LabelMatchScope.LABEL, key: constants_1.RouteMatchedLabel.KEY }],
            },
        },
    };
 ...
 */
function buildRuleName(routeIndex, policyIndex, action) {
    const prefix = `route-${routeIndex}-policy-${policyIndex}`;
    if (action === constants_1.RouteAction.BLOCK) {
        return `${prefix}-block`;
    }
    if (action === constants_1.RouteAction.FREE) {
        return `${prefix}-free`;
    }
 ...
 *
 */
function buildConditionStatement(condition) {
    // String → single LabelMatchStatement
    if (typeof condition === 'string') {
        return { labelMatchStatements: [{ scope: constants_1.LabelMatchScope.LABEL, key: condition }] };
    }
    // String array → AND of LabelMatchStatements (AND logic)
    if (Array.isArray(condition)) {
        const statements = condition.map(label => ({
            labelMatchStatements: [{ scope: constants_1.LabelMatchScope.LABEL, key: label }],
        }));
        return { andStatement: { statements } };
    }
    // { namespace: "<value>" } → single LabelMatchStatement with NAMESPACE scope
    if ('namespace' in condition) {
        return { labelMatchStatements: [{ scope: constants_1.LabelMatchScope.NAMESPACE, key: condition.namespace }] };
    }
    // { and: [...] }
    if ('and' in condition) {
 ...
 */
function buildStatement(uriStatement, condition) {
    // "default" condition — match any request to this URI path
    if (condition === constants_1.DefaultCondition.VALUE) {
        return uriStatement;
    }
...
function buildAction(action) {
    if (action === constants_1.RouteAction.BLOCK) {
        return constants_1.RouteAction.BLOCK;
    }
    return {
        insertHeader: {
            name: constants_2.Headers.ROUTE_ACTION,
            value: action,
        },
    };
}
//# sourceMappingURL=waf-rule-translator.js.map